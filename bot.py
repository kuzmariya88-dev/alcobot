...
async def handle_drinks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("drk_"):
        ...
        if drink == "done":
            if not user_sessions[user_id]['drinks']:
...
async def handle_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text("❌ Ошибка при расчете. Попробуйте еще раз.")
...
async def webhook_handler():
    try:
        ...
        if update.message and update.message.text == '/start':
            ...
            if query_data.startswith('evt_'):
                await handle_event_type(update, None)
            elif query_data.startswith('fmt_') or query_data.startswith('dur_'):
                await handle_format(update, None)
            elif query_data.startswith('drk_'):
                await handle_drinks(update, None)
            elif query_data.startswith('prc_'):
                await handle_price(update, None)
        elif update.message and update.message.text:
            await handle_guests(update, None)
